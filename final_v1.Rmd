...
## Data Description
We will use the bank churn data available on Kaggle
([ DataSource](https://www.kaggle.com/shrutimechlearn/churn-modelling)).
Churning from a bank means that the customers no longer do businesses with the
bank. So we will have a binary response variable indicating whether the
customer exited from the bank service or not.
The data contains 10000 observations of 14 variables of which 2000 are postive
observations. For simplicity, we only
...
[lfit](https://stats.idre.ucla.edu/stata/webbooks/logistic/chapter3/lesson-3-logistic-regression-diagnostics/) and 
[estat gof](https://www.stata.com/manuals13/restatgof.pdf) for performing
Hosmer-Lemeshow test. However, in order to make HL plot for model calibration,
we will first install the hl packages from
[sealedenvelope](https://www.sealedenvelope.com/).
...
core examples. Unlike likelihood based method, HL test doesn't provide any 
directions for model selection. It only tells us the goodness of fit of the
logistic model but doesn't provide any constructive suggestion to optimize it.

...
library(tidyverse)
library(jtools) # print model summary
library(ggplot2)
...
From this preliminary results, we can see that `IsActiveMember` has a significant negative coefficient. This indicated that being an active member reduced the probability of churn. The coefficients for `CreditScore`, `Balance` and `EstimatedSalary` are very close to zero, which is consistent with our previous results from the exploratory data analysis. There is no significant difference in terms of `CreditScore`, `Balance` and `EstimatedSalary` between exited and retained customers.

We performed backward elimination to reduce the complexity of the model. We first removed `CreditScore`, `Balance`and  `EstimatedSalary` from the model and refitted the model.
...
When comparing BIC of model 1 with that of model 2, we noticed BIC of the reduced model is greater than that of the full model by 6.
...
We have a non-significant p-values for this full model, which means that we have no evidence that the model is fitting poorly.

The first two columns are the expected numbers of 0's and 1's. The last two columns are the observed numbers of retained and exited customers. We divided the points into ten groups.

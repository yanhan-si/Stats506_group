...
* Figure 1 shows the distribution of credit scores stratified by the response variable `Exited` status. There is no vast difference between these two groups in terms of the credit scores distribution.

```{r}
data %>% 
  ggplot(aes( CreditScore, color = Exited )) + 
  geom_density() + 
  ggtitle("Figure.1 The distribution of credit score by exited") +
  theme_bw()
```
...
In this model fitting part, we will use logistic regression to predict the p
